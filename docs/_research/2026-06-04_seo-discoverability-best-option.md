...
> **CORRECTION (2026-06-04, post-research):** The research agents incorrectly claimed this repo is NOT in the HACS default store. **It IS** — `lasswellt/govee-homeassistant` is present in `hacs/default`'s `./integration` list, and `home-assistant/brands` already serves `custom_integrations/govee/` (`icon.png`, `icon@2x.png`, `logo.png`, `logo@2x.png`). So the "biggest lever / hard blocker" framing below is **already solved**. No brand icon to add, no default-store PR to file. Real remaining levers: star growth, community presence, on-page SEO (done: topics + headings), `awesome-home-assistant` listing (PR opened), and the strategic LAN feature.

~~The single biggest lever is **HACS default-store inclusion**~~ — already done (see correction). Most users never leave the HACS in-app search, and the repo **is** discoverable there. Discoverability (Google + community) is the active track: ~9 more GitHub topics (done), submit to `frenck/awesome-home-assistant` (done — PR #744), and post in the 228K-view legacy `LaggAt` forum thread as the maintained successor. Being "the best option" technically is largely already true for cloud multi-device control (AWS IoT MQTT push, 2FA, capability-based, broadest device-type coverage) — but the field leader `wez/govee2mqtt` (1,400★) wins on **LAN/offline control**, which this repo lacks. Closing the LAN gap + winning discoverability is the path to #1.
